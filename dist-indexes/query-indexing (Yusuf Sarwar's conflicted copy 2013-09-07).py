...
class Query:
	...
	def key(self):
		s = ''
		for k, v in self.dataitem.items():
			if type(v) is types.StringType and not any(i in "+-*" for i in v):
				s = s + k + v
		return s	
...
class Client:
	...
	def do(self, N):
		...
		for i in range(SIZE):
			print N, K, i, 'Q:', self.qoverlay.qnodes[i].count(), self.qoverlay.qnodes[i].size() / 1000.0, 'KB', 
		...
		for i in range(10):
			...
			#q = Query({'platform' : 'Windows8'})
			result = self.qoverlay.serveQuery(q)
			print "Query results: records", len(result)
...
def main():
	global K
	for K in [2, 3, 4, 5]:
		for N in range(1000, 10 * 1000, 1000):		
			client = Client()
			client.do(N)
